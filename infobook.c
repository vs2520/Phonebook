...
void namefun(){
	...
	double phone;
	char gender[8];
	fptr=fopen("contactBook.txt","ab+");//ab+ gives us the ability of writing the function and add the second data in the existing one...
	if(fptr==NULL){
...
void exitfun(){
	...
	gotoxy(31,6);
	printf("\xDB Vaibhav Sahu.");
	gotoxy(31,8);
	printf("\xDB Piyush Singh.");
	gotoxy(31,10);
	printf("\xDB Tapesh Dasila.");
	gotoxy(31,12);
	printf("\xDB Suraj Rawat.");
}
void password(void){
	char passwords[20]={"contactBook"};
	gotoxy(22,2);
	int j;
	int z;
